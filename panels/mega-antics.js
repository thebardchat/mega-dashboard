...
const MegaAntics = (() => {
    ...
    function init() {
        if (running) return;
        if (!window.MegaCrew) {
            console.warn('[MegaAntics] MegaCrew not found, cannot initialize antics.');
            return;
        }
        ...
        console.log(`[MegaAntics] v2.0 — ${characters.length} characters deployed across ${panels.length} panels. They live here now. yeehaw-ay!`);
    }
    ...
    function stop() {
        ...
        panels = [];
        characters = [];
        console.log('[MegaAntics] Stopped. The crew goes silent...');
    }
    ...
    function triggerAntic(name) {
        ...
        if (activity) {
            ...
            console.log(`[MegaAntics] Triggered: ${name} on ${char.memberId}`);
        }
    }
...
function initMegaAntics() {
    MegaAntics.init();
}

window.MegaAntics = MegaAntics;
